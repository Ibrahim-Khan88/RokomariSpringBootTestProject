...
public class DataService {
	...
	public String margeAudioAndVideo(MultipartFile audioFile, MultipartFile videoFile)
			throws IOException, InterruptedException {

		String videoFilePath, videoWithOutAudioPath, audioFilePath, finalVideoFilePath, convertMkvFormatFilePath,
				ffmpeg;
		dir += File.separator;
		ffmpeg = Loader.load(org.bytedeco.ffmpeg.ffmpeg.class);

		videoFilePath = dir + "inputVideo" + videoFile.getOriginalFilename()
				.substring(videoFile.getOriginalFilename().indexOf("."), videoFile.getOriginalFilename().length());
		audioFilePath = dir + "inputAudio" + audioFile.getOriginalFilename()
				.substring(audioFile.getOriginalFilename().indexOf("."), audioFile.getOriginalFilename().length());
		...
		ProcessBuilder process2 = new ProcessBuilder(ffmpeg, "-y", "-i", convertMkvFormatFilePath, "-an",
				videoWithOutAudioPath);
		process2.inheritIO().start().waitFor();

		ProcessBuilder process3 = new ProcessBuilder(ffmpeg, "-y", "-i", videoWithOutAudioPath, "-i", audioFilePath,
				finalVideoFilePath);
